...
type WebBuildUpdateProps = {
  show: boolean;
  webBuild?: WebBuildUpdated;
  onClose: () => void;
  onSave: () => void;
}
export default function WebBuildUpdate({
  show,
  webBuild,
  onClose,
  onSave = onClose,
  ...
  const shape = {
    old_slug: yup.string().required('Required'),
  ...
  const onSubmit : FormikConfig<typeof initialValues>['onSubmit'] = async (values, { resetForm }) => {
    ...
    const res = await withSchema(supabase, 'w4online').rpc('web_build_update', {
      slug: preparedValues.old_slug,
      new_slug: preparedValues.slug,
      name: preparedValues.name,
      object_id: preparedValues.object_id,
      public: preparedValues.public,
    })
    if (res.error) {
      toast.error(`Could not update web build: ${res.error.message}`)
      return
    }
    toast.success('Web build successfully updated')
    onSave()
    resetForm()
  }
      ...
      {({
        ...
        >
          ...
          <Form noValidate onSubmit={handleSubmit}>
            <Modal.Body>
              <input type="hidden" id="old_slug" name="old_slug" value={initialValues.old_slug} />
