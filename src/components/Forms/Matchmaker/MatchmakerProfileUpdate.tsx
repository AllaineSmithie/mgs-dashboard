...
export type MatchmakerProfileUpdated = {
  id: string;
  name: string;
  query: string;
  min_players: number;
...
type MatchmakerProfileUpdateProps = {
  show: boolean;
  updated?: MatchmakerProfileUpdated;
  onClose: () => void;
  onSave: () => void;
}
export default function MatchmakerProfileUpdate({
  show,
  updated,
  onClose,
  onSave = onClose,
}: MatchmakerProfileUpdateProps) {
  ...
  const shape = {
    ...
    lobby_cluster: yup.string().nullable().when('lobby_type', {
      is: 1,
  ...
  const initialValues = Object.assign(
    ...
    {
      ...updated,
      min_players: updated.min_players.toString(),
      max_players: updated.max_players.toString(),
      lobby_type: updated.lobby_type.toString(),
      lobby_state: updated.lobby_state.toString(),
    },
  )
  ...
  const onSubmit : FormikConfig<typeof initialValues>['onSubmit'] = async (values, { resetForm }) => {
    ...
    if (res.error) {
      toast.error(
        `Could not update matchmaking profile: ${res.error?.message}`,
      )
      return
    }
