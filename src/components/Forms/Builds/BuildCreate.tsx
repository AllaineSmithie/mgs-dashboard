  ...
  const onSubmit : FormikConfig<typeof initialValues>['onSubmit'] = async (values, { resetForm }) => {
    ...
    if (res.error) {
      if (res.error.code === '23505') {
        ...
        toast.error(
          'Could not create build: another build using the same file or the same name already exists',
        )
      } else {
        toast.error(`Could not create build: ${res.error.message}`)
      }
      return
    }
    toast.success('Build successfully created')
    onSave()
    resetForm()
  }
      ...
      {({
        ...
        >
          <Modal.Header closeButton={!isSubmitting}>
            <Modal.Title>Create a new build</Modal.Title>
          </Modal.Header>
